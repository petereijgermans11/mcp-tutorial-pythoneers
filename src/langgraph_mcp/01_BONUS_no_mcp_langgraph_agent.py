...
MATH_VALIDATION_PROMPT = """You are a classifier. Determine whether the user's question is ONLY about performing a math operation: addition, multiplication, or division.
...
Examples of NO:
- "Make a word document where you add 3 and 4"
- "What's the weather?"
- "Write an essay about addition"
...
def validate_math_question(state: MessageState):
    ...
    response = validation_llm.invoke(
        [HumanMessage(content=MATH_VALIDATION_PROMPT.format(question=user_message))]
    )
    is_math = response.content.strip().upper().startswith("YES")
    if not is_math:
        return {
            "messages": [
                AIMessage(
                    content="I can only answer questions about math operations (addition, multiplication, or division)."
                )
            ]
        }
    return {}
...
def route_after_validation(state: MessageState) -> Literal["assistant", "__end__"]:
...
def build_graph(tools):
    ...
    builder.add_node("validate_math_question", validate_math_question)
    builder.add_node("assistant", assistant)
    builder.add_node("tools", ToolNode(tools))
    # Define edges
    builder.add_edge(START, "validate_math_question")
    builder.add_conditional_edges("validate_math_question", route_after_validation)
...
if __name__ == "__main__":
    ...
    # Run the graph with the input state and the config from LangGraph
